...
async def dashboard(
    ...
    try:
        ...
        def _lookup(cp_id_val, field):
            return cp_lookup.get(str(cp_id_val), {}).get(field, "Unknown")
        ...
        def _is_negative(row):
            ...
            if stop is None or (isinstance(stop, float) and np.isnan(stop)):
                return is_charging == 0
            return stop == "Failed / Error"  # Only "Failed / Error" is negative, not "Incomplete"

        df["is_negative"] = df.apply(_is_negative, axis=1).astype(int)
        df["is_precharging_fail"] = (df["is_Charging"].fillna(0) == 0).astype(int)
        df["is_charging_fail"] = (
        ...
        def _compute_metrics(sub):
            ...
            positive = int((sub["stop"] == "Successful").sum())
            ...
            return {
                ...
                "prechargingFailures": int(sub["is_precharging_fail"].sum()),
                "networkPerformance": round(positive / total * 100, 2),
...
async def cpid_rankings(
    ...
    try:
        ...
        def _is_negative(row):
            stop = row.get("stop")
            if stop is None or (isinstance(stop, float) and np.isnan(stop)):
                return row.get("is_Charging", 0) == 0
            return stop == "Failed / Error"  # Only "Failed / Error" is negative, not "Incomplete"
        ...
        for cpid, grp in df.groupby("cp_id"):
            ...
            result.append({
                "cpid": str(cpid),
                "chargingSessions": total,
                "negativeStops": neg,
                "negativeStopPercentage": round(neg / total * 100, 2) if total > 0 else 0,
            })
